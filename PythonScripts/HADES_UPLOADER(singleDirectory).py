...
base_directory = r'C:\Users\Alek\OneDrive\Desktop\College\Iowa State\2024\Fall 2024\CYBE 491 - Sr. Design and Professionalism I\POSYDON\POSYDON DATA\POSYDON_data\single_HeMS'
...
# Find all H5 files in the directory (non-recursive)
h5_files = glob(os.path.join(base_directory, '.*h5'))
...
# Function to clean and make dataset and column names SQL-friendly
...
for file_path in h5_files:
    ...
    table_name = sanitize_sql_name(os.path.splitext(os.path.basename(file_path))[0])
            ...
            for dataset_name, dataset in dataset_list:
                try:
                    ...
                    if dataset.dtype.names is not None:
                        columns = [sanitize_sql_name(name.decode('utf-8') if isinstance(name, bytes) else name) for name
                                   in dataset.dtype.names]
                    elif data.shape[1] > 1:
                        columns = [f"Column_{i + 1}" for i in range(data.shape[1])]
                    else:
                        columns = [sanitize_sql_name(dataset_name.split('/')[-1])]
                    ...
                    try:
                        sanitized_table_name = f"{table_name}_{sanitize_sql_name(dataset_name.replace('/', '_'))}"
